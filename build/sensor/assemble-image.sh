...
if [ ! -f "${ROOTFS}" ] || [ ! -f "${KERNEL}" ]; then
    echo "ERROR: Build artifacts missing; run 'make -C build/buildroot-src O=../sensor/output sensor_defconfig && make -C build/buildroot-src O=../sensor/output' first" >&2
    exit 1
fi
...
# NOTE: Populating partitions requires loop device mounting (root/Docker).
# The image skeleton is created here; partition population must be done
# with appropriate privileges or in a privileged container.
cat <<'NOTE'
